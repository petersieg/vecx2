...
static char cartfilename[255] = "";
static char overfilename[255] = "";
...
int main(int argc, char *argv[]){
	char const * lFilterPatterns[1] = { "*.bin" };
	char * lTheOpenFileName;

	lTheOpenFileName = tinyfd_openFileDialog(
		"Choose game rom file",
		"./rom/",
		1,
		lFilterPatterns,
		"rom files",
	...
	SDL_Log("Selection was %s", lTheOpenFileName);
		sprintf(cartfilename,"%s",lTheOpenFileName);
		sprintf(overfilename,"%s.bmp",lTheOpenFileName);
		SDL_Log("cartfilename: %s", cartfilename);
		SDL_Log("overfilename: %s", overfilename);
